...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
	...
	long, _ := strconv.ParseFloat(r.URL.Query().Get("lon"), 64)
	ran := DISTANCE
	...
	query = query.Distance(ran).Lat(lat).Lon(long)
	searchResult, err := readFromES(query, POST_INDEX)
	if err != nil {
	...
	js, err := json.Marshal(posts)
	if err != nil {
		http.Error(w, "Failed to parse searchresult to JSON!", http.StatusInternalServerError)
		return
	}
	w.Write(js)
}
